...
impl CPU<'_> {
    pub fn new(memory: Memory, graphics: &mut Graphics) -> CPU {
        return CPU {
            graphics,
            speed: 500,
            v: [0; 16],
        ...
        match (op0, op1, op2, op3) {
            ...
            (0xD, x, y, n) => {
                ...
                for i in 0..n {
                    ...
                    self.graphics.set(self.v[x], self.v[y] + (i as u8), pixels);
                }
                println!("");
            }
    ...
    fn get_op(&mut self) -> u16 {
        // 获取当前CPU指令 并向后2 byte移动指针
        let op: u16 =
            ((self.memory.address[self.pc] as u16) << 8) + self.memory.address[self.pc + 1] as u16;
        return op;
    }
    fn next(&mut self) {
        self.pc += 2;
